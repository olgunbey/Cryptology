...
DiffieHellmanAlg diffieHellmanAlg = new DiffieHellmanAlg();
...
string AliceGonderilecekMesaj = "olgunbeyşahin123 ah sana vah sana arada yazar bana";
Aes aes = new Aes();
KeyAes keyAes = new KeyAes(aes);

AesEncrypt aesEncrypt = new AesEncrypt(aes, keyAes);

  byte[] EncryptData=  aesEncrypt.Encrypt(AliceGonderilecekMesaj, alice.SharedKey);
  
  Console.WriteLine("Şifrelenmiş mesaj {0}",BitConverter.ToString(EncryptData).Replace("-",""));
...
if (isVerifiedByBob)
{
    AesDecrypt aesDecrypt = new AesDecrypt(aes,keyAes);
    var data=aesDecrypt.Decrypt(EncryptData, alice.SharedKey);
    Console.WriteLine("Şifrelenmiş mesaj {0}",data);
}
...
public class Aes:IAes
{
   ...
   public byte InvSubByte(byte value)
   {
   ...
   {
       // InvMixColumns işlemi
       byte[,] invMixColumnsMatrix = {
           {0x0E, 0x0B, 0x0D, 0x09},
           {0x09, 0x0E, 0x0B, 0x0D},
           {0x0D, 0x09, 0x0E, 0x0B},
           {0x0B, 0x0D, 0x09, 0x0E}
       ...
       for (int i = 0; i < 4; i++)
       {
           for (int j = 0; j < 4; j++)
           {
               ...
               for (int k = 0; k < 4; k++)
               {
                   val ^= GaloisMultiply(invMixColumnsMatrix[i, k], state[k, j]);
               }
               result[i, j] = val;
           }
       }
...
public class AesEncrypt:IDisposable
{
    ...
    public byte[] Encrypt(string text, string keys)
    {
        ...
        // Metni uygun boyuta genişlet
        int paddedLength = (int)Math.Ceiling((double)textBytes.Length / blockSize) * blockSize;
        byte[] paddedText = new byte[paddedLength];
        Array.Copy(textBytes, paddedText, textBytes.Length);

        int Nb = 4; // Blok boyutu (32-bit kelimelerin sayısı)
        int Nr = 14; // Tur sayısı

        int blockCount = paddedText.Length / blockSize;
        byte[] encryptedData = new byte[paddedText.Length];
        ...
        for (int blockIndex = 0; blockIndex < blockCount; blockIndex++)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < Nb; j++)
                {
                    state[i, j] = paddedText[blockIndex * blockSize + i + 4 * j];
                }
            }
            ...
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < Nb; j++)
                {
                    encryptedData[blockIndex * blockSize + i + 4 * j] = state[i, j];
                }
            }
        }
 ...
 public class AesDecrypt : IDisposable
 {
     private readonly IAes _aes;
     private readonly IAesKey _aesKey;
     public AesDecrypt(IAes aes, IAesKey aesKey)
     {
         _aes = aes;
         _aesKey = aesKey;
     }
     public string Decrypt(byte[] encryptedData, string keys)
     {
         var key = Encoding.UTF8.GetBytes(keys);
         int blockSize = 16;
         ...
         for (int blockIndex = 0; blockIndex < blockCount; blockIndex++)
         {
             ...
             // Şifreli veriyi state matrisine kopyala
             for (int i = 0; i < 4; i++)
             {
                 for (int j = 0; j < Nb; j++)
                 {
                     state[i, j] = encryptedData[blockIndex * blockSize + i + 4 * j];
                 }
             }

             byte[,] roundKey = _aesKey.KeyExpansion(key, Nr);

             // Son tur anahtarı ile XOR işlemi (şifreleme sırasındaki son durumu tersine çevir)
             _aesKey.AddRoundKey(state, roundKey, Nr);

             for (int round = Nr - 1; round > 0; round--)
             {
                 _aes.InvShiftRows(state);
                 _aes.InvSubBytes(state);
                 _aesKey.AddRoundKey(state, roundKey, round);
                 _aes.InvMixColumns(state);
             }

             // İlk tur anahtarı ile XOR işlemi (şifreleme sırasındaki ilk durumu tersine çevir)
             _aes.InvShiftRows(state);
             _aes.InvSubBytes(state);
             _aesKey.AddRoundKey(state, roundKey, 0);

             // Çözülen veriyi byte dizisine dönüştür
             for (int i = 0; i < 4; i++)
             {
                 for (int j = 0; j < Nb; j++)
                 {
                     decryptedData[blockIndex * blockSize + i + 4 * j] = state[i, j];
                 }
             }
         }

         // Padding'i kaldır ve string olarak dönüştür
         int padding = decryptedData[decryptedData.Length - 1];
         byte[] unpaddedData = new byte[decryptedData.Length - padding];
